...
// 도착한 지 24시간이 지나도록 아직 열어보지 않은 받은 편지의 수신자에게 푸시한다.
//
// 중복 방지: 하루 1회 실행 + 도착 시각이 (now-48h, now-24h] 구간에 든 편지만 스캔.
// → 각 편지의 '도착 24시간 경과' 시점은 하루치 윈도 하나에만 들어가므로 한 번만 알림이 간다.
//   (사용자는 '도착 후 24~48시간' 사이에 한 통당 한 번의 미열람 리마인더를 받는다.)
...
async function handle(request: Request) {
  ...
  const olderThanTs = toDbTs(now - REMIND_AFTER_HOURS * 3600 * 1000) // 도착 24h 경과
  const newerThanTs = toDbTs(now - (REMIND_AFTER_HOURS + 24) * 3600 * 1000) // 단, 48h 이내(하루치 윈도)
